...
class _AssetAmount extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // 无地址：没有可查的对象，按 0 展示。
    if (address == null) return _amount(context, '0', 0);
    ...
    return balance.when(
      ...
      error: (_, _) => _amount(context, '0', 0),
      data: (b) => _amount(context, b.amount, b.fiatValue, utxo: b.utxo),
    );
  }

  /// 两行：上为持仓数量，下为折算法币价值。
  ///
  /// 数量后面不再缀币种符号：左侧主标题已经写了一遍，右侧重复只是把行挤长。
  ///
  ...
  Widget _amount(
    ...
    return Column(
      ...
      children: [
        AmountText.raw(formatTokenAmount(amount), style: theme.textTheme.bodyMedium),
