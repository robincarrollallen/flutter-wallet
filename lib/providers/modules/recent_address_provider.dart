...
class RecentAddressesNotifier extends Notifier<List<RecentAddress>>
    with PersistentNotifier<List<RecentAddress>> {
  ...
  @override
  Map<String, dynamic> toJson(List<RecentAddress> state) =>
      {'entries': state.map((e) => e.toJson()).toList()};

  @override
  List<RecentAddress> fromJson(
    Map<String, dynamic> json,
    List<RecentAddress> fallback,
  ) {
  ...
  void record(String chainId, String address) {
    final rest = state
        .where((e) => !(e.chainId == chainId && e.address == address))
        .toList();
    state = [
      RecentAddress(chainId: chainId, address: address),
      ...rest.take(_maxEntries - 1),
    ];
  }
}

final recentAddressesProvider =
    NotifierProvider<RecentAddressesNotifier, List<RecentAddress>>(
  RecentAddressesNotifier.new,
);

/// 指定链下的最近使用地址（保持最新在前）。
final recentAddressesOfChainProvider =
    Provider.family<List<String>, String>((ref, chainId) {
  return ref
