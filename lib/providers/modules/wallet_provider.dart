...
class WalletListNotifier extends Notifier<List<Wallet>> with PersistentNotifier<List<Wallet>> {
  ...
  void addBackupMethod(String id, BackupMethod method) {
    state = [
      for (final w in state)
        if (w.id == id) w.copyWith(backupMethods: {...w.backupMethods, method}) else w,
    ];
  }
...
/// 所有钱包。
final walletListProvider = NotifierProvider<WalletListNotifier, List<Wallet>>(WalletListNotifier.new);

/// 当前选中钱包的 id。
class CurrentWalletIdNotifier extends Notifier<String?> with PersistentNotifier<String?> {
  @override
