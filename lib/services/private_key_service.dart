...
class PrivateKeyService {
  ...
  static bool _isEvmHex(String s) => RegExp(r'^[0-9a-fA-F]{64}$').hasMatch(_strip0x(s));
  ...
  static DerivedWallet _deriveEvm(String s) {
    ...
    return DerivedWallet(addresses: addresses, primaryPrivateKey: '0x${hex.toLowerCase()}');
  }
  ...
  static DerivedWallet _deriveSolana(String s) {
    ...
    final chain = SupportedChains.all.firstWhere((c) => c.kind == ChainKind.solana);
  ...
  static String _strip0x(String s) => (s.startsWith('0x') || s.startsWith('0X')) ? s.substring(2) : s;
}
