...
class TronTransactionHistoryService implements ChainTransactionHistoryService {
  ...
  Future<TransactionHistoryPage> fetch({
    ...
    return parseTronPage(
      nativeResponse: responses[0],
  ...
  Future<Map<String, dynamic>> _request({
    ...
    final uri = Uri.parse('${chain.endpoint}/v1/accounts/$address/transactions$path')
        .replace(queryParameters: {'limit': '$limit', 'order_by': 'block_timestamp,desc', 'fingerprint': ?fingerprint});
    return getJson(uri);
  }
}
...
TransactionHistoryPage parseTronPage({
  ...
  final records = [
    for (final entry in _dataOf(nativeResponse))
      ?_parseNative(entry, chain: chain, address: address, walletId: walletId),
  ...
  return TransactionHistoryPage(
    records: records,
    nextCursor: exhausted ? null : '${nativeNext ?? ''}|${tokenNext ?? ''}',
  );
}
...
TransactionRecord? _parseNative(
