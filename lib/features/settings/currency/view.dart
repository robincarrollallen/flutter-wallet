...
class _CurrencyScreenState extends ConsumerState<CurrencyScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(backgroundColor: theme.colorScheme.inversePrimary, title: Text(t.currency.title)),
      body: Column(
        children: [
...
class _CurrencyTile extends StatelessWidget {
  const _CurrencyTile({required this.code, required this.name, required this.selected, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      leading: SizedBox(
        width: 32.s,
        child: Center(
          child: Text(currencyFlagOf(code), style: TextStyle(fontSize: 24.s)),
        ),
      ),
      title: Text(code, style: theme.textTheme.titleMedium),
      subtitle: Text(name, style: theme.textTheme.bodySmall?.copyWith(color: theme.colorScheme.onSurfaceVariant)),
