...
class WalletKeyService {
  ...
  ///
  /// 注意：这是给「用户查看 / 外部钱包回导」的编码格式。真实签名应改用原始
  /// 32 字节私钥 / 账户对象，避免编码-解码往返（后续实现签名时再补对应方法）。
...
/// 全局单例 provider，供导出私钥 / 签名等流程注入使用。
final walletKeyServiceProvider = Provider<WalletKeyService>(
  (ref) => WalletKeyService(ref.read(secureWalletStorageProvider)),
);
