...
class _AssetTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final chainLogoUrl =
        chainIcons[asset.chain.coinGeckoPlatformId] ??
        markets[asset.chain.coinGeckoId]?.logoUrl;
    return ListTile(
      leading: AssetIcon(
        symbol: asset.symbol,
      ...
      // 附带链名，便于区分同符号原生币（如多条 EVM 链的 ETH）。
      subtitle: Text('${asset.name} · ${asset.chain.name}'),
      // 右侧展示持仓「数量 + 折算价值」。
