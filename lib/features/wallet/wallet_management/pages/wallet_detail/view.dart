...
class WalletDetailPage extends ConsumerWidget {
                    ...
                    children: [
                          ...
                          children: [
                            ...
                            Positioned(
                              ...
                              child: Container(
                                ...
                                child: Icon(Icons.edit_note, size: 12.s, color: theme.colorScheme.onPrimary),
                              ),
                            ),
                          ],
                        ),
                      ...
                      Row(
                        ...
                        children: [
                          Text(
                            current.name,
                            style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600, fontSize: 18.s),
                          ),
                          IconButton(
  ...
  /// 删除钱包：二次确认后移除并退出详情页。
  Future<void> _onDelete(BuildContext context, WidgetRef ref, Wallet current) async {
  ...
  void _onBackup(BuildContext context, Wallet current) {
    Navigator.of(context).push(panelSlideRoute(BackupMethodPage(wallet: current)));
  }

  /// 进入导出私钥流程：列出钱包支持的所有链。
  void _onExportPrivateKey(BuildContext context, Wallet current) {
    Navigator.of(context).push(panelSlideRoute(ExportPrivateKeyPage(wallet: current)));
  }
                ...
                Wrap(
                  ...
                  children: [
                    for (final option in WalletAvatarCatalog.all)
                      GestureDetector(
                        onTap: () {
                          ref.read(walletListProvider.notifier).setIcon(current.id, option.key);
  ...
  /// 弹出改名对话框，确认后写入并持久化。
  Future<void> _showRenameDialog(BuildContext context, WidgetRef ref, Wallet current) async {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        ...
        TextButton(onPressed: () => Navigator.of(context).pop(_controller.text), child: const Text('保存')),
      ],
    );
  }
}
