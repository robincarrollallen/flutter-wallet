...
class EvmTransactionService {
  ...
  Future<({String hash, String sentAmount})> sendNative({
    ...
    final nonceHex = await jsonRpcCall(chain.endpoint, EvmRpcMethod.getTransactionCount.wireName, [from.address, 'pending']) as String;
    // 估算费用
    ...
    final balanceHex = await jsonRpcCall(chain.endpoint, EvmRpcMethod.getBalance.wireName, [from.address, 'latest']) as String;
    ...
    final hash = await jsonRpcCall(chain.endpoint, EvmRpcMethod.sendRawTransaction.wireName, ['0x${_toHex(raw)}']) as String;
...
class _EvmGasFee {
  const _EvmGasFee.eip1559({required BigInt this.maxFeePerGas, required BigInt this.maxPriorityFeePerGas}) : eip1559 = true, gasPrice = null;
