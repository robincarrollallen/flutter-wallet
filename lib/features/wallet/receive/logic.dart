...
class ReceiveLogic {
  ...
  /// 代币来自 [catalog]。
  static List<ListedAsset> assetsOf(Chain? chain, TokenCatalog catalog) => ListedAsset.fromCatalog(catalog, chain: chain);
  ...
  static List<ListedAsset> filter(List<ListedAsset> assets, String query) {
    ...
    return assets.where((a) {
      return a.symbol.toLowerCase().contains(q) || a.name.toLowerCase().contains(q) || a.chain.name.toLowerCase().contains(q);
    }).toList();
  }
}
