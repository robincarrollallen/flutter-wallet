...
class ImportMnemonicLogic {
  ...
  static SecretType detectType(String input) {
    ...
    if (lower.startsWith('0x') || lower.startsWith('suiprivkey') || trimmed.length >= 40) {
  ...
  static List<String> suggestions(String text) {
    ...
    if (wordlist.contains(prefix) && !wordlist.any((w) => w != prefix && w.startsWith(prefix))) {
      return const [];
    }
    return wordlist.where((w) => w.startsWith(prefix)).take(maxSuggestions).toList(growable: false);
  }
  ...
  static MnemonicError? validate(String input) {
    ...
    final unknown = words.where((w) => !MnemonicService.englishWordlist.contains(w)).toList();
    if (unknown.isNotEmpty) {
      return MnemonicError(MnemonicErrorKind.invalidWords, words: unknown.take(3).toList());
    }
