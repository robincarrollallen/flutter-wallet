...
void main() {
  ...
  group('SendLogic.partition', () {
    ...
    test('价值降序，零值进 rest 并保持原顺序', () {
      final values = {
        'ethereum-sepolia': 10.0,
        'solana-devnet': 30.0,
        'bsc-testnet': 20.0,
      };
      final (sendable, rest) = SendLogic.partition(
        assets,
        (a) => values[a.chain.id] ?? 0,
      );
      expect(sendable.map((a) => a.chain.id).toList(), [
        'solana-devnet',
        'bsc-testnet',
        'ethereum-sepolia',
      ]);
      ...
      expect(
        ...
        assets
            .where((a) => !values.containsKey(a.chain.id))
            .map((a) => a.chain.id)
            .toList(),
      );
    });
  ...
  group('SendLogic.validateAddress', () {
    ...
    test('Sui/Aptos：32 字节十六进制', () {
      expect(
        SendLogic.validateAddress(SupportedChains.suiTestnet, '0x2'),
        isNull,
      );
      expect(
        SendLogic.validateAddress(SupportedChains.aptosTestnet, '0x'),
        isNotNull,
      );
    });
  });
