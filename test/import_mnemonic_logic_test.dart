...
void main() {
  ...
  group('applySuggestion', () {
    test('替换末尾单词并补空格', () {
      expect(ImportMnemonicLogic.applySuggestion('aba ab', 'abandon'), 'aba abandon ');
    });
  });

  group('validate', () {
    MnemonicErrorKind? kind(String input) => ImportMnemonicLogic.validate(input)?.kind;
    ...
    test('含无效单词', () {
      const input = 'abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon zzzz';
      expect(kind(input), MnemonicErrorKind.invalidWords);
    });

    test('词数与字符合法但校验和错误', () {
      const input = 'abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon';
